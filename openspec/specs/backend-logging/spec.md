...
Structured operational logging for backend routers and services, covering provider CRUD events, API test requests, collection operations, stats queries, and scheduler job lifecycle. Uses Python standard `logging` module with `"backend.*"` namespace.
